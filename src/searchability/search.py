...
from .models import CandidateSet, Receipt, SearchResult, VectorRecord, as_float32_vector
...
class SearchEngine:
    ...
    ) -> list[tuple[Group, tuple[VectorRecord, ...], np.ndarray]]:
    ...
    def search_full_scan(
        ...
        *,
        ...
        candidates = candidate_set or self.prepare_candidates(
        ...
        receipt = Receipt(
            ...
            fallback_reason="population_below_k" if population_small else None,
            numeric_mode=NUMERIC_MODE,
            component_timings={
                "base_search_ns": candidates.search_ns,
                "visibility_ns": visibility_ns,
                "raw_scan_ns": scan_ns if raw else 0,
                "lb_calculation_ns": 0,
                "group_ordering_ns": 0,
                "group_scan_ns": scan_ns if visible_groups else 0,
                "merge_ns": merge_ns,
                "receipt_ns": 0,
            },
            distance_evaluations=len(records),
            exact_boundary_rechecks=ranked.exact_rechecks,
            certificate_details={"authoritative_gap": "0/1"} if not population_small else None,
        ...
        receipt = Receipt(**{**receipt.to_dict(), "component_timings": timings})
    ...
    def search_pruned(
        ...
        *,
        ...
        raw_ns = time.perf_counter_ns() - raw_start
                ...
                audit_rows.append(
                    {
                        ...
                        "tau_upper_at_decision": None if not math.isfinite(tau_upper) else tau_upper,
        ...
        merge_start = time.perf_counter_ns()
        scanned_records, scanned_sources = _deduplicate(scanned_records, scanned_sources)
        final_matrix = (
            np.ascontiguousarray(
                np.stack([record.vector for record in scanned_records]), dtype=np.float32
            )
        ...
        certificate_details: dict[str, object] | None
        if population_small:
            certified_beta = None
            min_skipped_lb = None
            ...
            certificate_details = {"authoritative_gap": "0/1"}
        else:
            ...
            beta_fraction = Fraction.from_float(beta)
            certificate_details = {
                "tau_upper_fraction": f"{tau_fraction.numerator}/{tau_fraction.denominator}",
        ...
        receipt = Receipt(
            ...
            distance_evaluations=len(scanned_records) + len(bounded_groups),
            exact_boundary_rechecks=ranked.exact_rechecks,
